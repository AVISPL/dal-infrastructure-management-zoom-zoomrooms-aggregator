    ...
    public void controlProperty(ControllableProperty controllableProperty) throws Exception {
        ...
        if(!StringUtils.isNullOrEmpty(roomId) && !StringUtils.isNullOrEmpty(property)){
            if(property.startsWith("RoomSettings#")) {
                String setting = ZoomRoomsSettings.valueOf(property.split("#")[1]).name();
                ...
                switch (property) {
                    case "LeaveCurrentMeeting":
                        ...
                        break;
                    case "EndCurrentMeeting":
                        endCurrentMeeting(roomId);
                        break;
                    case "RestartZoomRoomsClient":
...
//            aggregatedDeviceProcessor.applyProperties(settingsProperties, settingsControls, meetingSettings, "RoomAlertSettings");
            ...
            if(devices != null && devices.isArray()) {
                for(JsonNode deviceNode: devices) {
                    ...
                    room.getProperties().putAll(properties);
                }
            }

            if(room.getProperties().get("RoomStatus").equals("InMeeting")) {
                room.getProperties().put("EndCurrentMeeting", "");
                room.getControllableProperties().add(createButton("EndCurrentMeeting", "End", "Ending...", 0L));

                room.getProperties().put("LeaveCurrentMeeting", "");
                room.getControllableProperties().add(createButton("LeaveCurrentMeeting", "Leave", "Leaving...", 0L));
            }

            if(!room.getProperties().get("RoomStatus").equals("Offline")) {
                room.getProperties().put("RestartZoomRoomsClient", "");
                room.getControllableProperties().add(createButton("RestartZoomRoomsClient", "Restart", "Restarting...", 0L));
            }
    ...
    private JsonNode retrieveRoomProfile(String roomId) throws Exception {
        JsonNode roomProfile = doGet(String.format(ZOOM_ROOM_PROFILE_URL, roomId), JsonNode.class);
        return roomProfile;
    }
...
//        if (StringUtils.isNullOrEmpty(aggregatorProperties)) {
//            throw new InstantiationError("Unable to retrieve multiple statistics. Please make sure 'aggregatorProperties' parameter is set.");
//        }
//        JsonNode deviceProperties = objectMapper.readTree(aggregatorProperties);
//        Map<String, String> properties = new HashMap<>();
//        aggregatedDeviceProcessor.applyProperties(properties, deviceProperties, "Aggregator");
